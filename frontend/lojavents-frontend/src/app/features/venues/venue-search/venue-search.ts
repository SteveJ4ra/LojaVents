...
export class VenueSearch {
  readonly filters = signal({ eventType: '', date: '', attendees: 1, maxPrice: 150, text: '' });
  ...
  constructor(
    ...
    this.form.valueChanges.subscribe(value => {
      const next = { ...initial, ...value } as typeof initial;
  ...
  reset(): void {
    this.form.setValue({ eventType: '', date: '', attendees: 1, maxPrice: 150, text: '' });
  }
  ...
  toggleFavorite(id: string): void {
    ...
    this.notifications.show(added ? 'Local guardado.' : 'Local eliminado.', 'success');
  }
}
